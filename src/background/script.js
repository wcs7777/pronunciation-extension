...
async function setMenuItem(accessKey) {
  ...
  const id = "P";
  setMenuItemPromise = browser.menus
    .remove(id)
    .catch(() => {})
    .finally(() => {
...
async function setShowPlayerMenuItem(show) {
  ...
  return browser.menus.update(showPlayerMenuItem.id, {
...
async function installedCB(details) {
  ...
  if (details.reason === "install" || details.temporary) {
    ...
    const [major, minor, bug] = details.previousVersion
      .split(".")
      .map(parseInt);
    if (major < 3) {
      // break change
...
async function localStorageOnChangedCB(changes) {
  try {
    ...
    const ipaKeys = changesKeys.filter((k) => k.startsWith(st.ipaTable.name));
    const audioKeys = changesKeys.filter((k) =>
      k.startsWith(st.audioTable.name),
    );
    const optionsKeys = changesKeys.filter((k) =>
      k.startsWith(st.optionsTable.name),
    );
    if (ipaKeys.length > 0) {
