export async function migrateToV3_5_0() {
  console.log("migrating options to v3.5.0");
  ...
  for (const source of Object.keys(options.audio.sources)) {
    delete options.audio.sources[source]["saveError"];
  }
...
export async function migrateToV3() {
  ...
  const options = {
        ...
        elevenLabs: {
          ...
          api: {
            ...
            applyTextNormalization:
              old.audio.elevenLabs.api.applyTextNormalization,
          },
        },
        amazonPolly: {
    ...
    setPronuncationByShortcut: {
      ...
      restoreDefaultIpaShortcut:
        old.setPronuncationByShortcut.restoreDefaultIpaShortcut,
    },
  };
  await browser.storage.local.set({ options });
}
