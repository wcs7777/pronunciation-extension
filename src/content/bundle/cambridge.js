...
`;
  ...
  let TableByKeyPrefix$1 = class TableByKeyPrefix {
    ...
    async get(keys) {
      ...
      return Object.entries(values).reduce((withoutKeyPrefix, [key, value]) => {
      ...
      try {
        ...
        const cb = {
          [options.ipaShortcut]: async () => {
            const rawIpa = document.querySelector("span.ipa").textContent.trim();
            const ipa = `/${rawIpa}/`;
          ...
          [options.restoreDefaultIpaShortcut]: async () => {
            const currentIpa = await ipaTable.getValue(word, false);
            const defaultIpa = await defaultIpaTable.getValue(word, false);
            if (defaultIpa) {
