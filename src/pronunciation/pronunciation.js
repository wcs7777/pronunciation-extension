...
export default class Pronunciation {
  ...
  async pronounce() {
    ...
    if (!this.pi.isText) {
      ...
      await Promise.all([
        ...
        this.showIpa(this.fetchIpa(this.options.ipa), this.options.ipa),
      ]);
    } else {
      ...
      await Promise.all([
        ...
        this.showIpa(this.fetchIpaText(this.options.ipa), this.options.ipa),
      ]);
    }
  }
   ...
   * @returns {Promise<void>}
   */
  async showLastErrors(lastErrors, { top = 100, closeTimeout = 5000 } = {}) {
    try {
      for (const le of lastErrors) {
        await this.showInfo(le, {
          top,
          closeTimeout,
        });
