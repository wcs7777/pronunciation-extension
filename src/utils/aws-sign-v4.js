export class AwsFetcher {
  ...
  async calculateSignature() {
    ...
    const dateRegionKey = await sign(dateKey, this.region);
    const dateRegionServiceKey = await sign(dateRegionKey, this.service);
    const signingKey = await sign(dateRegionServiceKey, "aws4_request");
    const signature = await sign(signingKey, await this.createStringToSign());
  ...
  get canonicalQueryString() {
    ...
    this.#canonicalQueryString = Array.from(this.url.searchParams.entries())
      .map(([key, value]) => {
  ...
  get canonicalHeaders() {
    ...
    this.#canonicalHeaders =
      this.canonicalHeadersEntries.map(([k, v]) => `${k}:${v}`).join("\n") +
      "\n";
    return this.#canonicalHeaders;
  }
  ...
  get service() {
    ...
    this.#service = this.awsOpt.service ?? this.url.hostname.split(".", 1)[0];
    return this.#service;
  }
  ...
  get region() {
    ...
    "".split();
    this.#region = this.awsOpt.region ?? this.url.hostname.split(".", 2)[1];
    return this.#region;
  }
  ...
  get canonicalHeadersEntries() {
    ...
    this.#canonicalHeadersEntries = Object.entries(this.headersToSign)
      .map(([key, value]) => {
...
function uriEncode(uri, isPath = true) {
  const encoded = encodeURIComponent(uri).replace(
    /[!'()*]/g,
