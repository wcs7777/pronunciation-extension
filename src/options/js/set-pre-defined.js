...
el.ipaShort.addEventListener("click", async () => {
  try {
    const msg =
      "Are you sure to set short pre-defined IPA definitions (some definitions may be overrode and may take some time)?";
...
el.ipaLong.addEventListener("click", async () => {
  try {
    const msg =
      "Are you sure to set long pre-defined IPA definitions (some definitions may be overrode and may take some time)?";
    if (!window.confirm(msg)) {
      return;
    }
