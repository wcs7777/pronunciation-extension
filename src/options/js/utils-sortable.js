...
export function createSortableOrder(list, dataIdAttrSuffix = "order") {
  ...
  return Sortable.create(list, {
    ...
    forceFallback: true,
    onEnd: () => {
      const children = Array.from(list.querySelectorAll(`[${dataIdAttr}]`));
      for (const [index, element] of children.entries()) {
        const order = index + 1;
        element.dataset[datasetKey] = order.toString().padStart(2, "0");
      }
    },
  });
}
 ...
 * @returns {void}
 */
export function sortSortableOrder(
  sortable,
  items,
  initialOrder,
  dataIdAttrSuffix = "order",
) {
  const datasetKey = kebab2camel(dataIdAttrSuffix);
  for (const [key, value] of Object.entries(items)) {
    if (key in initialOrder) {
      const order = initialOrder[key][datasetKey];
      value.dataset[datasetKey] = order.toString().padStart(2, "0");
    }
  }
