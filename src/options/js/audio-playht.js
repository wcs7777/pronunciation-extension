...
document.addEventListener("DOMContentLoaded", async () => {
  try {
    [el.sampleRate, el.temperature].forEach((e) => onlyNumber(e, true));
        ...
        sources: {
          playHt: {
            api: {
              ...
              voiceId: strOr(
                strOr(el.voiceIdNotListed.value, el.voiceId.value),
                defaultApi.voiceId,
              ),
              quality: strOr(el.quality.value, defaultApi.quality),
              outputFormat: strOr(
                el.outputFormat.value,
                defaultApi.outputFormat,
              ),
              sampleRate: numOr(
                el.sampleRate.value,
                defaultApi.sampleRate,
                8000,
                48000,
              ),
              temperature: numOr(
                el.temperature.value,
                defaultApi.temperature,
                0,
                2,
              ),
